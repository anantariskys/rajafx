...
const LayananXMCard: FC<Props> = ({ description, linkText, title, variant = "primary", link }) => {
  ...
  return (
    ...
    >
      ...
      {link ? (
        <Link href={link} className="relative cursor-pointer gap-1 group-hover:gap-2 duration-300 ease-in-out z-20 text-white font-medium flex items-center">
          <p>{linkText}</p>
          <Icon icon="material-symbols:arrow-right-alt-rounded" />
        </Link>
      ) : (
        <div className="relative opacity-50 cursor-default gap-1 z-20 text-white font-medium flex items-center">
          <p>{linkText}</p>
          <Icon icon="material-symbols:arrow-right-alt-rounded" />
        </div>
      )}
